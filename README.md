...
Let's consider how steps 2–6 above map onto the API/MCP stages below. Step 1 (reusing saved content) is mostly UI/Library-only for `sumo_search_client.py` and `sumosearch` — both are scoped to the Search Job API — except for dashboard content specifically: `sumo_dashboard_client.py`/`sumosearch` can find a relevant dashboard and mine its panels' query text via the Dashboard Report Job API (see the new row below and [`skills/discovery-dashboard-reuse`](skills/discovery-dashboard-reuse/SKILL.md)). Reusing saved *searches* or *alerts* directly still isn't exposed by either. Sumo's own MCP investigator skill can reuse dashboards and alerts as first-class objects (list/create, not query-text extraction) directly (see the table's last row); that's a Sumo-provided capability layered on MCP, not something this repo's client or CLI expose.

| Stage | `sumo_search_client.py` | `sumosearch` CLI | Sumo MCP |
| --- | --- | --- | --- |
| **Reuse** dashboard content (find a relevant dashboard, extract panel query text) | `sumo_dashboard_client.py`'s `list_dashboards()`/`get_dashboard()`, summarized via `cli/dashboard_describe.py`'s `describe_dashboard_queries()` | `sumosearch discover dashboards --match\|--grep` + `report describe --queries` | Not exposed by `runLogSearch`/`listPartitions`/`listExtractionRules`; only via the separate Investigator skill's `listDashboards` (object reuse, not query-text extraction) |
...
| Stage | Skill(s) |
| --- | --- |
| **Reuse** dashboard content | [`discovery-dashboard-reuse`](skills/discovery-dashboard-reuse/SKILL.md) |
